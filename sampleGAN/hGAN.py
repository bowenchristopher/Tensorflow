...
def imagePrepare(argv):
    ...
    newImage = Image.new('L', (int(width), int(height)), (255))
    if width > height:
        nheight = int(round((width / width * height), 0))
        if (nheight == 0):
            nheight == 1
        img = im.resize((int(width), nheight), Image.ANTIALIAS).filter(ImageFilter.SHARPEN)
        wtop = int(round(((int(width) - nheight) / 2), 0))
        newImage.paste(img, (4, wtop))
    else:
        nwidth = int(round((width / width * height), 0))
        if (nwidth == 0):
            nwidth == 1
        img = im.resize((int(width), nwidth), Image.ANTIALIAS).filter(ImageFilter.SHARPEN)
        wleft = int(round(((int(width) - nwidth) / 2), 0))
...
directory = 'sampleGAN/data/floral/'
for filename in os.listdir(directory):
    if filename.endswith(".jpeg"):
        test.append(imagePrepare(directory + filename))
...
# read in .jpg images from "data_set/"
# resize to 28 by 28
...
def plot(samples):
    ...
    for i, sample in enumerate(samples):
        ...
        ax.set_yticklabels([])
        ax.set_aspect('equal')
        plt.imshow(sample.reshape(50, 50), interpolation=interpolation)
...
# Rotated image
X_train2 = X_train[half:].reshape(-1, 50, 50)
X_train2 = scipy.ndimage.interpolation.rotate(X_train2, 90, axes=(1, 2))
X_train2 = X_train2.reshape(-1, 50*50)
# Cleanup
del X_train
...
for it in range(5000):
    ...
    if it % 1000 == 0:
        ...
        input_A = sample_X(X_train1, size=5)
        input_B = sample_X(X_train2, size=5)
